  ...
  private void onImagePicked(@NonNull final byte[] imageBytes) {
    ...
    new AsyncTask<Void, Void, ClarifaiResponse<List<ClarifaiOutput<Concept>>>>() {
      ...
      @Override protected void onPostExecute(ClarifaiResponse<List<ClarifaiOutput<Concept>>> response) {
        ...
        steps.setText("");
        int cnt=1;
        // water rodents recyclable non recyclable
        boolean[] arr={false,false,false,false};
        String[] values={"Use disinfectant on water.\nClean water from there using gloves. (Refer Here)","Use rodent control techniques. (Refer Here)",
                "Filter recyclable materials. (Refer Here)","Handle other kind of waste. (Refer Here)"};
        for(int i=0;i<predictions.get(0).data().size();i++){
          Concept concept=predictions.get(0).data().get(i);
          //steps.setText(steps.getText().toString()+concept.name()+"\n");
          String str=concept.name();
          if(str.equals("water") || str.equals("wet")){
            arr[0]=true;
          }
          if(str.equals("insect") || str.equals("invertebrate")){
            arr[1]=true;
          }
          if(str.equals("recycling") || str.equals("junk") || str.equals("junkyard")){
            arr[2]=true;
          }
          if(str.equals("trash") || str.equals("garbage") || str.equals("waste")){
            arr[3]=true;
          }
        }
        for(int i=0;i<4;i++){
          if(arr[i]){
            steps.setText(steps.getText().toString()+"Step "+cnt+": "+values[i]+"\n");
            cnt++;
          }
        }
      }
  ...
  private void setBusy(final boolean busy) {
    runOnUiThread(new Runnable() {
      @Override public void run() {
        ...
        steps.setVisibility(busy?GONE:VISIBLE);
